...
A DubSync plugin rendszere hét fő plugin típust támogat:
...
7. **Language Plugin-ek**: Új felületi nyelvek hozzáadása
...
A DubSync beépített nemzetköziesítési rendszert biztosít, amelyet a pluginok használhatnak több nyelv támogatásához. **A pluginoknak saját `locales/` mappával kell rendelkezniük** - nem módosíthatják az alkalmazás központi locale fájljait.

### Plugin Locale Fájl Struktúra
...
├── requirements.txt  # Függőségek (opcionális)
└── locales/          # Plugin fordítások (kötelező i18n-hez)
    ├── en.json       # Angol fordítások (kötelező - fallback)
    └── hu.json       # Magyar fordítások
```

### Locale Fájlok Létrehozása

Hozz létre JSON fájlokat a plugin `locales/` mappájában:

**locales/en.json** (Angol - kötelező fallback-ként):
```json
{
  "name": "My Plugin",
...
class MyPlugin(UIPlugin):
    ...
    def info(self) -> PluginInfo:
        return PluginInfo(
            id="my_plugin",  # Ez az ID szerepel a fordítási kulcsokban
            # ...
        )
    
    def create_dock_widget(self) -> QDockWidget:
        # Használd a t()-t "plugins.{plugin_id}.{key}" mintával
        dock = QDockWidget(t("plugins.my_plugin.panel_title"))
        
        widget = QWidget()
        layout = QVBoxLayout(widget)
        
        # Minden UI szövegnek t()-t kell használnia
...
### Automatikus Locale Betöltés

A plugin locale-ok **automatikusan betöltődnek** a plugin inicializálásakor. Az alap `PluginInterface.initialize()` metódus kezeli ezt. Ha felülírod az `initialize()` metódust, hívd meg a `super().initialize()`-t:

```python
def initialize(self) -> bool:
    """Plugin inicializálása."""
    super().initialize()  # Ez tölti be a locale fájlokat a locales/ mappából
    # Egyéni inicializálás...
    return True
```

### Fordítási kulcs elnevezési konvenció

| Kulcs minta | Használat |
|-------------|----------|
| `plugins.{id}.name` | Plugin megjelenített neve |
| `plugins.{id}.description` | Plugin leírása |
| `plugins.{id}.panel` | Dock panel címe |
| `plugins.{id}.menu_*` | Menüelem címkék |
| `plugins.{id}.status_*` | Státusz üzenetek |
| `plugins.{id}.*_btn` | Gomb címkék |
| `plugins.{id}.*_tooltip` | Tooltip szövegek |
| `plugins.{id}.*_placeholder` | Input placeholder-ek |

### Paraméteres fordítások

Használd a Python `.format()` metódusát dinamikus értékekhez:

```python
# A locale fájlban:
# "items_found": "{count} elem találva"
# "error_message": "Hiba: {error}"
...
- **Angol (en)** - Alapértelmezett/fallback nyelv
- **Magyar (hu)**
...
7. **Nemzetköziesítés**: Használd a `t()` függvényt minden UI szöveghez
8. **Locale kulcsok**: Kövesd a `plugins.{plugin_id}.{key}` elnevezési konvenciót
9. **Mindkét nyelv**: Add hozzá a fordításokat az `en.json` és `hu.json` fájlokhoz is
...
| Plugin | Típus | Leírás |
|--------|-------|--------|
| [csv_export](../src/dubsync/plugins/builtin/csv_export/) | Export | CSV exportálás |
| [basic_qa](../src/dubsync/plugins/builtin/basic_qa/) | QA | Alapvető ellenőrzések |
| [glossary](../src/dubsync/plugins/builtin/glossary/) | UI | Fordítási szószedet |
| [translator](../src/dubsync/plugins/builtin/translator/) | UI + Service | Argos fordító |
| [spellchecker](../src/dubsync/plugins/builtin/spellchecker/) | UI | Helyesírás-ellenőrző |
